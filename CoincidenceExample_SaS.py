...
class CoincidenceExample(QMainWindow):
    ...
    def __init__(self, tagger):
        ...
        self.ui = uic.loadUi(os.path.join(my_dir, 'CoincidenceExampleWindow_SaS.ui'), self)
        ...
        self.ui.delayC.valueChanged.connect(
            lambda value: self.setInputDelay('C', value)
        )
        self.ui.delayD.valueChanged.connect(
            lambda value: self.setInputDelay('D', value)
        )
        ...
        self.ui.triggerC.valueChanged.connect(
            lambda value: self.setTriggerLevel('C', value)
        )
        self.ui.triggerD.valueChanged.connect(
            lambda value: self.setTriggerLevel('D', value)
        )
        ...
        self.ui.testsignalC.stateChanged.connect(
            lambda state: self.setTestSignal('C', state != 0)
        )
        self.ui.testsignalD.stateChanged.connect(
            lambda state: self.setTestSignal('D', state != 0)
        )
    ...
    def updateMeasurements(self):
        ...
        self.tagger.setTestSignal(channels[2], self.ui.testsignalC.isChecked())
        self.tagger.setTestSignal(channels[3], self.ui.testsignalD.isChecked())
        ...
        #     ch0=0
        # if  self.ui.testsignalB.isChecked() is TRUE:
        #     ch0=1
        # if  self.ui.testsignalA.isChecked() is TRUE:
            ch0=0
        ...
        # # Measure the correlation between C and D
        # self.correlation = Correlation(
        #     self.tagger,
        #     # channels[1],
        #     # channels[0],
        ...
        #     print(ch)
        #     # self.correlation=Correlation(sm.getTagger(), channels[0], ch, self.ui.correlationBinwidth.value(),self.ui.correlationBins.value())
    ...
    def getTaggerChannel(self, label):
        ...
        assert label in 'ABCD', 'Unknown channel label "{}"'.format(label)
    ...
    def saveClicked(self):
        ...
        if filename:
            with open(filename, 'w') as f:
                ...
                f.write('Test signal C: %d\n' %
                        self.ui.testsignalC.isChecked())
                f.write('Test signal D: %d\n' %
                        self.ui.testsignalD.isChecked())
